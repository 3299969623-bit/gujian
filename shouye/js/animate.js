...
if (!canvas) {
    console.warn('[Particle] 未找到 canvas#particle，粒子背景已跳过');
} else {
    (function () {
        ...
        const particles = [];
        for (let i = 0; i < 120; i++) particles.push({ x: Math.random() * w, y: Math.random() * h, r: Math.random() * 1.5 + 1, vx: Math.random() * .4 - .2, vy: Math.random() * .4 - .2 });
        function draw() {
            ctx.clearRect(0, 0, w, h);
            ctx.fillStyle = 'rgba(0,246,255,.05)';
            particles.forEach(p => {
...
if (typeof gsap !== 'undefined') {
    ...
    if (document.querySelector('.top-bar')) {
        timeline.from('.top-bar', { y: -100, opacity: 0, duration: 1, ease: 'power3.out' });
    }
    
    if (document.querySelector('.kpi-item')) {
        timeline.from('.kpi-item', { scale: .8, opacity: 0, duration: .8, stagger: .1 }, '-=.5');
    }
    
    if (document.querySelector('.panel')) {
        timeline.from('.panel', { scale: .9, opacity: 0, duration: 1, stagger: .15 }, '-=.6');
    }
} else {
    console.warn('[GSAP] 库未加载，入场动画已跳过');
}
